define(["tools/hash", "text!templates/popup/error.html"], function(hashlib, msg_template){
    ...
    var STORAGE_KEY = 'LS_USER_MESSAGES'
        , $modalContainer = $(window.__options__.$modalContainer)
        , $messagingContainer = $(window.__options__.$messagingContainer)
        ...
        , MessageView = Backbone.View.extend({
            ...
            , template:_.template(msg_template)
        ...
        , view = new View({el: $messagingContainer})
        ...
        , showModal = function(html){
            var events = {}
            events[hnc.support.clickEvent] = function(e){
                var $t = $(e.target);
                if($t.is(".close, .modal-container")){
                    $modalContainer.off().empty().hide();
                }
            };
            $modalContainer.html(html).show().on(events);
        };
