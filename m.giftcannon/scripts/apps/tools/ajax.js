  ...
  , ajax = {
      ...
      , submit: function(options){
              ...
              params.success = function (resp, status, xhr) {
                  ajax._in_error = false;
                  ajax.trigger("network:online");
                  if (resp.redirect) {
              ...
              /*params.error = function(xhr, errorType, error){
                  ajax.trigger("network:error");
                  if(options._retry || !ajax._in_error){
                      ajax._in_error = true;
                      if((options._retry||0)<3){
                          options._retry = (options._retry||0)+1;
                          var confText = "There seems to be a problem with the network. Retry";
                          if(options._retry)confText=confText+"("+options._retry+"/3)?"
                          else confText=confText+"?";
                          if(confirm(confText))ajax.submit(options);
                          else if (options.error) options.error.apply(this, arguments);
                      } else window.location.reload(true);
                  } else
                    ajax.on("network:online", function(){
                        options._retry = (options._retry||0)+1;
                        ajax.submit(options);
                        ajax.off("network:online");
                    });
              };
              */
              if(params.data && params.data.Gift)params.data.Gift.deviceId = deviceId;
              else if(params.data)params.data.deviceId = deviceId;
              else params.data = {deviceId:deviceId};
